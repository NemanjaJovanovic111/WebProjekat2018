...
public class VolunteerDAO {
	...
	public Volunteer createVolunteer(String username, String password, String email, String firstName, String lastName,
		...
		Volunteer volunteer = new Volunteer(username, password, email, firstName, lastName, profilePicture, phoneNumber,
				territory);
		...
		for (Volunteer v : volunteers) {
			for (Volunteer updated : updatedVolunters) {
				if (updated.getUsername().equals(v.getUsername())) {
					v.setUserStatus(updated.getUserStatus());
				}
			}
	...
	public Volunteer getOne(String username) {
		for (Volunteer v : volunteers) {
